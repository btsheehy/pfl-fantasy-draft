		...
		<div className="space-y-8">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
				...
				{topPlayersByPosition.map((pos) => (
					<div key={pos.position} className="bg-white rounded-lg shadow-md overflow-hidden">
						<div className="bg-gradient-to-r from-blue-600 to-blue-800 text-white py-4 px-6">
							<h3 className="text-xl font-semibold">Top Available {pos.position}s</h3>
						</div>
						<div className="p-6">
					...
					{otherTeams.map(team => (
						<div key={team.id} className="bg-white rounded-lg shadow-md overflow-hidden">
							...
							<div className="p-4">
								...
								<p className="text-sm mb-1">Remaining Cap: ${team.availableCap}</p>
								<p className="text-sm">Cap to spend on one player: ${team.capToSpendOnOnePlayer}</p>
							</div>
						</div>
					))}
				</div>
